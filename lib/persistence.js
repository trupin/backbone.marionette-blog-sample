...
exports.Model.prototype.toJSON = function () {
    return JSON.stringify(_.extend(this.storedData || {}, this.data || {}), ' ', 2);
};
...
exports.Collection.prototype.post = function (data, callback) {
    this.collection.insert(data, function (err, res) {
        if (err)
            return callback(err);
        return callback(null, new this.Model(res, { created: true, collection: this }));
    }.bind(this));
};
...
exports.Collection.prototype.get = function (id, callback) {
    ...
    this.collection.findOne(req, function (err, doc) {
        if (err)
            return callback(err);
        if (!doc)
            return callback(new Error('The model with id "' + this.index + '" = "' + id + '" doesn\'t exists.'));
...
exports.Collection.prototype.delete = function (model, callback) {
    ...
    this.collection.findAndRemove(_.pick(model.storedData, this.index), [], function (err) {
        callback(err);
    });
};